...
| FR-02 | Every inbound message has BodyLength (9) and CheckSum (10) validated, and messages are framed from the TCP stream however the bytes arrive. A garbled message (bad framing, length, checksum or non-ASCII) is ignored. An intact message with an invalid field (bad tag number, empty value, repeated tag) still passes the sequence and CompID checks, then gets a session Reject (373=0, 4 or 13) and uses up its MsgSeqNum. A wrong BeginString or a missing/invalid MsgSeqNum ends the session with Logout. | A tampered message produces no response; an empty 58= gets 373=4 and the session continues; messages separated by newlines or split at any byte are all framed. | BR-1 |
...
| FR-20 | A valid message type the router does not support gets BusinessMessageReject (35=j, 380=3, 372=MsgType). A MsgType that FIX 4.4 does not define gets session Reject 373=11. An inbound 35=j is not answered. | 35=V gets 35=j with 372=V; 35=ZZ gets 35=3 with 373=11. | BR-3 |
