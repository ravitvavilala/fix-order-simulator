...
class SmartOrderRouter:
    ...
    def plan(self, order: Order, qty: int, limit: float | None) -> list[tuple[Venue, float, int]]:
        ...
        for v in self.venues.values():
            for price, available in v.levels(order.instrument.key, order.is_buy, limit, order.owner):
    ...
    def route(self, order: Order, qty: int, limit: float | None, rest_remainder: bool) -> RouteResult:
        ...
        for venue, price, take in self.plan(order, qty, limit):
            ...
            fills = venue.submit(ref, order.order_id, order.owner, order.instrument.key, order.is_buy, take, price,
        ...
        listing = [v for v in self.venues.values() if v.lists(order.instrument.key)]
        if remaining and rest_remainder and limit is not None and listing:
            venue = min(listing, key=lambda v: (-v.make_rebate, v.mic))
            ref = self._new_ref(order)
            fills = venue.submit(ref, order.order_id, order.owner, order.instrument.key, order.is_buy, remaining, limit,
                                 rest_remainder=True)
            result.fills.extend(f for f in fills if f.parent_id is not None)
    ...
    def resting_qty(self, order: Order) -> int:
        """Quantity this order has resting, read from the venues' books themselves."""
        return sum(v.resting_for_parent(order.order_id) for v in self.venues.values())
